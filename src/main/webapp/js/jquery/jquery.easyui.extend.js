...
function extendToNum(numObj, treeObj, type) {
    if (type == "treeGrid") {
        var lTree = numObj.val();
        if (lTree != num && lTree != "" && lTree != undefined) {
            var split = lTree.split(",");
            var num = split[0];
            if (split.length > 1) {
                lTree = lTree.substring(lTree.indexOf(",") + 1, lTree.length);
                numObj.val(lTree);
                treeObj.treegrid('expand', num);
            } else if (split.length == 1) {
                treeObj.treegrid('expand', num);
                treeObj.treegrid("select", num);
            }
        }
    } else if (type == "tree") {
        var lTree = numObj.val();
        if (lTree != num && lTree != "" && lTree != undefined) {
            ...
            if (split.length > 0) {
                lTree = lTree.substring(lTree.indexOf(",") + 1, lTree.length);
                numObj.val(lTree);
                var node = treeObj.combotree('tree').tree('find', num);
                if (node != null && node != undefined) {
                    treeObj.combotree('tree').tree('expand',node.target);
                    treeObj.combotree('tree').tree('select',node.target);
                }
            }
        }
    }
