//图库管理
function initWebUploader(ctx, filePicker, fileList,fileUuid,fileType) {
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server : ctx + '/file/picture/upload',
		// 选择文件的按钮。可选。
		...
		success : function(data) {
			if (data.length != 0) {
				for (var i = 0; i < data.length; i++) {
					...
					$img = $li.find('img')
					$img.attr('src', data[i].filePath).width(100).height(100);
	...
	uploader.on('uploadSuccess', function(file, response) {
		...
		$('#' + file.id).find("img").attr('src',response.obj.filePath).width(100).height(100);
	});
...
// 删除图片
function deletePic(ctx, fileId) {
	progressLoad();
	$.ajax({
		type : "POST",
		url : ctx + '/file/fileDelete',
		data : {
			fileId : fileId
		},
		dataType : "json",
