...
public class InUserController {
	...
	public ModelAndView listPage(ModelAndView modelAndView){
		...
		modelAndView.setViewName("ncov/inuser/userList");
		return modelAndView;
	}

	@RequestMapping("listNoPage")
	public ModelAndView listnoPage(ModelAndView modelAndView){
		List<NcovRegionBuildingsEntity>  Buildings = ncovBuildingService.getListNcovBuildings();
		modelAndView.addObject("Buildings", Buildings);
		modelAndView.setViewName("ncov/inuser/norecorduserList");
		return modelAndView;
	}
	...
	public ModelAndView addPage(ModelAndView modelAndView){
		...
		modelAndView.setViewName("ncov/inuser/userAdd");
		return modelAndView;
	}
	...
	public ModelAndView editPage(ModelAndView modelAndView,Long id){
		...
		List<RoleDto> roleDtos = roleService.getListRole();
		modelAndView.addObject("roleDtos", roleDtos);
		modelAndView.setViewName("ncov/inuser/userEdit");
		return modelAndView;
	}
	...
	@RequestMapping("/dataNoGrid")
	@ResponseBody
	public PageModel dataNoGrid(HttpServletRequest request, PageModel pageModel, HttpSession session,NcovUsersCondition ncovUsersCondition){
		try {
			SessionInfo sessionInfo = (SessionInfo) session.getAttribute(CommonContants.SESSION_INFO);
			ncovUsersCondition.setSessionid(sessionInfo.getId());
			ncovUsersCondition.setUSERTYPE(Integer.parseInt(request.getParameter("usertype")));
			return ncovUsersService.dataNoGrid(ncovUsersCondition);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return pageModel;
